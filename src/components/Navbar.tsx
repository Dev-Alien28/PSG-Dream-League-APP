  ...
  return (
    <>
      <style>{`
        ...
        .psg-nav-item.active { color: #c4a050; }
        ...
        .psg-nav-item:active { transform: scale(0.9); }
        .psg-nav-label {
          font-size: 9px;
          font-weight: 600;
        ...
        .psg-nav-icon-wrap { position: relative; }
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`psg-nav-item${active ? ' active' : ''}`}>
              <span className="psg-nav-icon-wrap">{item.icon(active)}</span>
